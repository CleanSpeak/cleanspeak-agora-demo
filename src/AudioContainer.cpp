...
std::string AudioContainer::getWaveBase64() const {
	boost::filesystem::path ffmpeg = bp::search_path("ffmpeg");
	...
	// Stream data to ffmpeg and save to a file. (It has to be able to seek to write the header when its done, which is not possible with a pipe)
	bp::child c(ffmpeg, "-f", "s16le", "-ar", "48000", "-i", "pipe:0", "-ar", "16000", "-y", "out.wav",
	            bp::std_in < out);
	...
	// Open the file and seek to the end
	std::ifstream in("out.wav", std::ios_base::in | std::ios_base::binary | std::ios_base::ate);
